...
static struct whisper_full_params build_transcribe_params(
        int num_threads,
        int audio_ctx,
        bool single_segment,
        bool no_timestamps,
...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_fullTranscribe(
    ...
    struct whisper_full_params params = build_transcribe_params(
            num_threads,
            768,
            false,
            false,
...
JNIEXPORT void JNICALL
Java_com_whispercpp_whisper_WhisperLib_00024Companion_fullTranscribeStreaming(
        JNIEnv *env,
        jobject thiz,
        jlong context_ptr,
        jint num_threads,
    ...
    struct whisper_full_params params = build_transcribe_params(
            num_threads,
            audio_ctx,
            single_segment == JNI_TRUE,
            no_timestamps == JNI_TRUE,
            max_tokens
    );

    whisper_reset_timings(context);

    LOGI("About to run whisper_full streaming");
    if (whisper_full(context, params, audio_data_arr, audio_data_length) != 0) {
        LOGI("Failed to run streaming model");
    } else {
        whisper_print_timings(context);
    }
    (*env)->ReleaseFloatArrayElements(env, audio_data, audio_data_arr, JNI_ABORT);
